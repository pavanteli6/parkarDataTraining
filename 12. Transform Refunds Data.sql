...
SELECT refund_id,
       ...
       refund_amount,
       refund_reason
  FROM pavan_catalog_all.bronze.refunds;
...
-- CREATE SCHEMA pavan_catalog_all.silver;

-- COMMAND ----------

CREATE TABLE pavan_catalog_all.silver.refunds
AS
SELECT refund_id,
       payment_id,
       CAST(date_format(refund_timestamp, 'yyyy-MM-dd') AS DATE) AS refund_date,
...
SELECT * FROM pavan_catalog_all.silver.refunds;

-- COMMAND ----------

DESC EXTENDED pavan_catalog_all.silver.refunds;
